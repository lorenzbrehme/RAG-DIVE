...
rag_name = "doc-5_300_bge_bge_qwen4B-NEW"

output_file = "./data/conversation_data_" + rag_name + "_" + model_name + "_turns_" + str(n) + "_conversation_" +str(max)+ ".jsonl"
log_file = "./data/conversation_data_" + rag_name + "_" + model_name + "_turns_" + str(n) + "_conversation_" +str(max)+ ".log"
...
def getNextDocument(title):
    #open md file as string
    
    
    with open("../single-hop-RAG-dataset/corpus/" + title, "r", encoding="utf-8") as f:
        md_content = f.read()
...
def get_follow_up_question(answer, title):
    ...
    history_follow_up = model.get_chat_history()
    ...
    if (response.get('rag_input') is None or response.get('type') is None or response.get('question') is None or response.get('answer') is None):
            return None
    validation = conversation_validator.validate_follow_up_question_all_in_one(response, history_follow_up)
    if validation and not validation['correct']:
...
def generate_conversation():
    ...
    while counter < len(list_titles):
        ...
        history.append({'role': 'user', 'content': question["rag_input"]})
        if not failed:
            for i in range(n):
                if failed:
